...
from app.event_utils import track_event_if_configured
from app.kernel_agents.agent_base import BaseAgent
from utils_date import format_date_for_user
from app.models.messages_kernel import (ActionRequest, AgentMessage, AgentType,
...
class GroupChatManager(BaseAgent):
    ...
    async def create(
        ...
        **kwargs,
        ...
        if isinstance(agent_name, dict):
            agent_name = agent_name.get("name", str(agent_name))
        ...
        try:
            ...
            if isinstance(system_message, dict):
                instructions_str = str(system_message)
            elif system_message is not None:
                instructions_str = system_message
            else:
                instructions_str = cls.default_system_message(agent_name)

            agent_definition = await cls._create_azure_ai_agent_definition(
                agent_name=agent_name if agent_name is not None else AgentType.GROUP_CHAT_MANAGER.value,
                instructions=instructions_str,  # Pass the formatted string, not an object
                temperature=0.0,
                response_format=None,
            )

            if memory_store is None or not isinstance(memory_store, CosmosMemoryContext):
            ...
            return cls(
                ...
                memory_store=memory_store,
                tools=tools if isinstance(tools, list) or tools is None else None,
                system_message=str(system_message) if system_message is not None and not isinstance(system_message, dict) else cls.default_system_message(agent_name),
                agent_name=str(agent_name) if agent_name is not None else AgentType.GROUP_CHAT_MANAGER.value,
                agent_tools_list=agent_tools_list if isinstance(agent_tools_list, list) or agent_tools_list is None else None,
                agent_instances=agent_instances if isinstance(agent_instances, dict) or agent_instances is None else None,
                client=client,
    ...
    async def handle_input_task(self, message: InputTask) -> Plan:
        ...
        if hasattr(planner_agent, "handle_input_task") and callable(getattr(planner_agent, "handle_input_task", None)):
            result = await planner_agent.handle_input_task(message)
            logging.info(f"Plan created: {result}")
            return result
        elif hasattr(planner_agent, "process_input_task") and callable(getattr(planner_agent, "process_input_task", None)):
            result = await planner_agent.process_input_task(message)
            logging.info(f"Plan created using process_input_task: {result}")
            return result
        else:
            logging.error("Planner agent does not implement 'handle_input_task' or 'process_input_task'.")
            raise AttributeError("Planner agent does not implement 'handle_input_task' or 'process_input_task'.")
    ...
    async def handle_human_feedback(self, message: HumanFeedback) -> None:
        ...
        if plan is not None and getattr(plan, "human_clarification_response", None):
    ...
    async def _execute_step(self, session_id: str, step: Step):
        ...
        if plan is not None:
            ...
            formatted_string += "No plan information available.\n\n"
        formatted_string += (
