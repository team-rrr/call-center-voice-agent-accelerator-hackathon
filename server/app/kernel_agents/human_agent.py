...
from app.models.messages_kernel import (AgentMessage, AgentType,
...
class HumanAgent(BaseAgent):
    ...
    async def create(
        ...
        **kwargs,
        ...
        """Asynchronously create the HumanAgent.
        ...
        if memory_store is None:
            raise ValueError("memory_store must not be None")

        # Ensure agent_name is a string, fallback to default if not
        if not isinstance(agent_name, str) or not agent_name:
            agent_name = AgentType.HUMAN.value

        # Ensure system_message is a string, fallback to default if not
    ...
    async def handle_human_feedback(self, human_feedback: HumanFeedback) -> str:
        ...
        # Get the step
        if human_feedback.step_id is None:
            return "Step ID must not be None"
