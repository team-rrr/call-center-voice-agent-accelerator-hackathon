...
from app.kernel_tools.hr_tools import HrTools
from app.kernel_tools.marketing_tools import MarketingTools
from app.kernel_tools.procurement_tools import ProcurementTools
from app.kernel_tools.product_tools import ProductTools
from app.kernel_tools.tech_support_tools import TechSupportTools
from app.models.messages_kernel import (
    AgentMessage,
    AgentType,
    HumanFeedbackStatus,
    InputTask,
...
class PlannerAgent(BaseAgent):
    ...
    async def create(
        ...
        **kwargs: Any,
        ...
        if memory_store is None or not isinstance(memory_store, CosmosMemoryContext):
            raise ValueError("memory_store must be provided and must be of type CosmosMemoryContext")
    ...
    async def _create_structured_plan(
        ...
        try:
            ...
            for step_data in steps_data:
                ...
                # Ensure agent_name is of type AgentType
                if not isinstance(agent_name, AgentType):
                    try:
                        agent_enum = AgentType(agent_name)
                    except ValueError:
                        agent_enum = AgentType.GENERIC
                else:
                    agent_enum = agent_name
            ...
            dummy_step = Step(
                ...
                agent=AgentType.GENERIC,  # Pass the enum member, not its value
                status=StepStatus.planned,
                human_approval_status=HumanFeedbackStatus.requested,
                timestamp=datetime.datetime.utcnow(),
                data_type="step",
    ...
    def _generate_args(self, objective: str) -> Dict[str, Any]:
