...
from app.models.messages_kernel import (ActionRequest, ActionResponse,
...
class BaseAgent(AzureAIAgent):
    ...
    def __init__(
        ...
        # Ensure client is always an AIProjectClient instance
        if client is None:
            client = config.get_ai_project_client()

        # Call AzureAIAgent constructor with required client and definition
        # Ensure definition is not None and is of type Agent before passing
        if definition is None:
            raise ValueError("The 'definition' argument must be provided and must be of type 'Agent'.")
        # Convert tools to plugins if necessary, or pass None if not applicable
        plugins = None
        if tools:
            # Pass KernelFunction instances directly to plugins
            plugins = [tool for tool in tools]

        super().__init__(
            deployment_name=None,  # Set as needed
            plugins=plugins,  # Use the loaded plugins or None
        ...
        try:
            ...
            await self._memory_store.add_item(
                AgentMessage(
                    ...
                    step_id=action_request.step_id,
                    data_type="agent_message",  # Add the required data_type argument
                )
            )
    ...
    def save_state(self) -> Mapping[str, Any]:
        ...
        # If CosmosMemoryContext does not have save_state, just return an empty dict or relevant state
        return {"memory": None}

    def load_state(self, state: Mapping[str, Any]) -> None:
        """Load the state of this agent."""
        # CosmosMemoryContext does not support loading state; no action needed.
        pass
